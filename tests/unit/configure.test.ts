...
describe("sigil > configure", () => {
	it("registers the provider and writes the config it reads", async () => {
		...
		expect(providers).toEqual(["@c9up/sigil/provider"]);
		expect(files.map((f) => f.path)).toEqual(["config/hash.ts"]);
	});
	...
	it("declares the environment variables the config reads", async () => {
		...
		expect(env).toHaveProperty("HASH_DRIVER");
	...
	it("writes a config that imports from the package it configures", async () => {
		...
		expect(files[0]?.content).toContain("from '@c9up/sigil'");
	});
});
